...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        ip_allowed, ip_remaining = self.limiter.check_rate_limit(
            f"ip:{client_ip}", self.limit, self.window_seconds
        )
        if not ip_allowed:
            log.warning("rate_limit_rejected", scope="ip", client_ip=client_ip, limit=self.limit)
            return self._rate_limited_response()

        remaining = ip_remaining
        user_id = self._extract_user_id(request)
        if user_id is not None:
            user_allowed, user_remaining = self.limiter.check_rate_limit(
                f"user:{user_id}", self.limit, self.window_seconds
            )
            if not user_allowed:
                log.warning("rate_limit_rejected", scope="user", user_id=user_id, limit=self.limit)
                return self._rate_limited_response()
